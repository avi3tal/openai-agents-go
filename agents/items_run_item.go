...
func TResponseInputItemFromToolCallItemType(input ToolCallItemType) TResponseInputItem {
	...
	case ResponseCodeInterpreterToolCall:
		return TResponseInputItemFromResponseCodeInterpreterToolCall(v)
	case ResponseFunctionWebSearch:
		return openaitypes.ResponseInputItemUnionParamFromResponseFunctionWebSearch(responses.ResponseFunctionWebSearch(v))
	default:
...
func TResponseInputItemFromResponseCodeInterpreterToolCall(input ResponseCodeInterpreterToolCall) TResponseInputItem {
	call := responses.ResponseCodeInterpreterToolCall(input)
	paramValue := responses.ResponseCodeInterpreterToolCallParam{
		ID:          call.ID,
		ContainerID: call.ContainerID,
		Status:      call.Status,
		Type:        constant.ValueOf[constant.CodeInterpreterCall](),
		Outputs:     mapCodeInterpreterOutputs(call.Outputs),
	}
	if strings.TrimSpace(call.Code) != "" {
		paramValue.Code = param.NewOpt(call.Code)
	}
	return responses.ResponseInputItemUnionParam{
		OfCodeInterpreterCall: &paramValue,
	}
}

func mapCodeInterpreterOutputs(outputs []responses.ResponseCodeInterpreterToolCallOutputUnion) []responses.ResponseCodeInterpreterToolCallOutputUnionParam {
	if len(outputs) == 0 {
		return nil
	}
	params := make([]responses.ResponseCodeInterpreterToolCallOutputUnionParam, 0, len(outputs))
	for _, output := range outputs {
		switch strings.ToLower(strings.TrimSpace(output.Type)) {
		case "logs":
			params = append(params, responses.ResponseCodeInterpreterToolCallOutputUnionParam{
				OfLogs: &responses.ResponseCodeInterpreterToolCallOutputLogsParam{
					Logs: output.Logs,
					Type: constant.ValueOf[constant.Logs](),
				},
			})
		case "image":
			params = append(params, responses.ResponseCodeInterpreterToolCallOutputUnionParam{
				OfImage: &responses.ResponseCodeInterpreterToolCallOutputImageParam{
					URL:  output.URL,
					Type: constant.ValueOf[constant.Image](),
				},
			})
		default:
			// Skip unknown output types to avoid corrupting session data.
			continue
		}
	}
	if len(params) == 0 {
